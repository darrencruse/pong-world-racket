...
;; whalesong doesn't handle on-release it seems(?)
...
;; since whalesong doesn't support on-release I've made the below stop the paddle
;; movement when the user taps their up or down key a second time.
(define (handle-key-down world a-key)
  (cond
    [(key=? a-key "w")
      (if (eq? 0 (paddle-speed (pong-world-left-paddle world))) 
    ...
    [(key=? a-key "s")
      (if (eq? 0 (paddle-speed (pong-world-left-paddle world))) 
        ...
        (set-left-paddle world (stop-paddle (pong-world-left-paddle world))))]
    [(key=? a-key "up")
      (if (eq? 0 (paddle-speed (pong-world-right-paddle world)))  
    ...
    [(key=? a-key "down")
      (if (eq? 0 (paddle-speed (pong-world-right-paddle world))) 
        ...
        (set-right-paddle world (stop-paddle (pong-world-right-paddle world))))]
    ;; add escape and space here since whalesong doesn't seem to support on-release
    [(key=? a-key "escape") 
...
;; whalesong's big-bang doesn't seem to support on-release?
;; I wonder how hard it would be to add?  
;; (the paddle movement is pretty painful without it) 
