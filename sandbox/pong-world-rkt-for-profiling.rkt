...
;; at the moment the only reason I've made this racket (as opposed to BSL)
;; version was so I could try the profiler which uses "profile-thunk" i.e.
;; where a "thunk" is a no argument function but BSL doesn't support those
#lang racket
...
;; for debugging I displayed some values where the scores normally are:
;;      (list (text (number->string (ball-speed (pong-world-ball world))) 98 "white")
;;          (text (number->string (posn-x (ball-pos (pong-world-ball world)))) 98 "white"))
      (place-image
...
(define (draw-pong-world world)
  (if (myprint (string-append "draw " (number->string (random 10)) "\n"))
  (cond 
